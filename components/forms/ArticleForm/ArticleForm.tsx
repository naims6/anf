...
import { FileText, Layers, Loader2, Check, ChevronRight, Tag } from "lucide-react"
...
interface ArticleFormProps {
  mode: "create" | "edit"
  defaultValues: ArticleFormFields
  onSubmit: (data: ArticleFormFields) => Promise<void>
...
export function ArticleForm({
  ...
  const [selectedCatPath, setSelectedCatPath] = useState(initialCategoryPath || "")
  const [catSelectorOpen, setCatSelectorOpen] = useState(false)

  useEffect(() => {
    if (initialCategoryPath) {
      setSelectedCatPath(initialCategoryPath)
    }
  }, [initialCategoryPath])
      ...
      <Card className="shadow-xs hover:shadow-md transition-all duration-300 border-border/80 rounded-2xl">
        <CardHeader className="border-b px-6 py-4 bg-muted/10">
          ...
          <CardDescription className="text-xs">
            {mode === "create"
              ? "Provide the title, select a category, and write the article content."
        ...
        <CardContent className="p-6 space-y-5">
          ...
          {/* Category Selector */}
          <div className="space-y-2">
            <Label className="text-sm font-semibold">{sa("categoryLabel")}</Label>
              ...
              <>
                {!hasSelectedCategory && !catSelectorOpen && (
                  ...
                  >
                    ...
                    <span className="font-medium">{sa("categoryPlaceholder")}</span>
                  </button>
                )}
                        ...
                        {columns.map((colItems, colIdx) => (
                          ...
                          >
                            <div className="bg-muted/40 border-b px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">
                              {colIdx === 0 ? sa("categoryLabel") : `Sub ${colIdx}`}
                            </div>
          ...
          {/* Actions */}
          <div className={`flex justify-end pt-4 border-t border-border/60 ${onCancel ? "gap-3" : ""}`}>
            {onCancel && (
              <Button
                type="button"
                variant="outline"
